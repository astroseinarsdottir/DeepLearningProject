...
class Storage():
    ...
    def compute_return_advantage(self):
        ...
        for i in reversed(range(self.num_steps)):
            delta = (self.reward[i] + self.gamma * self.value[i+1]
                     * (1 - self.done[i])) - self.value[i]
            advantage = self.gamma * self.lmbda * \
                advantage * (1 - self.done[i]) + delta
...
class TransposeFrame(VecEnvWrapper):
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=255, shape=(
            obs_shape[2], obs_shape[0], obs_shape[1]), dtype=np.float32)
