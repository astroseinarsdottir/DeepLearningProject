...
from model_impala import Flatten, Encoder, Policy
import argparse
import numpy as np
...
class Train:
    ...
    def __init__(
        ...
        self.env = make_env(num_envs, num_levels=num_levels, env_name="coinrun",
                            distribution_mode=distribution_mode, seed_levels=seed_levels)
        ...
        self.optimizer = torch.optim.Adam(
            self.policy.parameters(), lr=1e-4, eps=1e-5, weight_decay=0.0001)

        # Define temporary storage
        # we use this to collect transitions during each iteration
        self.storage = Storage(
            self.env.observation_space.shape, self.num_steps, self.num_envs)
    ...
    def clipped_value_loss(self, b_value, new_value, reward):
        ...
        clipped_val = b_value + \
            (new_value - b_value).clamp(-self.eps, self.eps)
    ...
    def evaluate_policy(self):
        ...
        for _ in range(self.num_steps):
            ...
            # Store data
            self.storage.store(self.obs, action, reward,
    ...
    def train_policy_batch(self, batch):
        ...
        pi_loss = self.clipped_policy_loss(
            b_log_prob, new_log_prob, b_advantage)
    ...
    def save_reward_csv(self):
        df_current = {'Step': self.step,
                      'Average_Reward': self.storage.get_reward().item()}
        ...
        if (self.step > self.save_step):
            ...
            plt.savefig(self.run_name +
                        '/last_captured_reward_step_.png', format="png")
            plt.show()
            plt.close()
...
def main():
    ...
    parser.add_argument("--run_name", type=str, default="Run_" +
                        str(np.random.random_integers(1, 1e5)), help="", required=False)
    parser.add_argument("--distribution_mode", type=str,
                        default="easy", help="", required=False)
    parser.add_argument("--seed_levels", type=int,
                        default=0, help="", required=False)
    parser.add_argument("--total_steps", type=float,
                        default=8e6, help="", required=False)
    parser.add_argument("--num_envs", type=int, default=32,
                        help="", required=False)
    parser.add_argument("--num_levels", type=int,
                        default=10, help="", required=False)
    parser.add_argument("--num_steps", type=int,
                        default=256, help="", required=False)
    parser.add_argument("--num_epochs", type=int,
                        default=3, help="", required=False)
    parser.add_argument("--batch_size", type=int,
                        default=512, help="", required=False)
    parser.add_argument("--eps", type=float, default=0.2,
                        help="", required=False)
    parser.add_argument("--grad_eps", type=float,
                        default=0.5, help="", required=False)
    parser.add_argument("--value_coef", type=float,
                        default=0.5, help="", required=False)
    parser.add_argument("--entropy_coef", type=float,
                        default=0.01, help="", required=False)
